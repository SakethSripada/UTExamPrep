...
public class TestRunner {
    ...
    private static void runCases() {
        for (int caseIndex = 0; caseIndex < 6; caseIndex++) {
            ...
            Map sMap = new LinkedHashMap() {{ put("0", 65); put("10", 66); put("11", 256); }};
            ...
            BitInputStream sBits = new BitInputStream(0, 1, 0, 1, 1);
            BitInputStream oBits = new BitInputStream(0, 1, 0, 1, 1);
            ...
            Outcome studentOutcome = Outcome.capture(() -> { student.uncompress(sMap, sBits, sOutput); return null; });
            Outcome oracleOutcome = Outcome.capture(() -> { oracle.uncompress(oMap, oBits, oOutput); return null; });
