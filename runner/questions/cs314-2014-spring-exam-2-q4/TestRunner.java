...
public class TestRunner {
    ...
    private static void runCases() {
        for (int caseIndex = 0; caseIndex < 6; caseIndex++) {
            ...
            Map sMap = new LinkedHashMap() {{ put("A", new ArrayList<>(Arrays.asList("B"))); put("B", new ArrayList<>(Arrays.asList("A", "C"))); put("C", new ArrayList<>(Arrays.asList("B"))); }};
            ...
            Outcome studentOutcome = Outcome.capture(() -> student.examCall(sMap, colors, sStrings, amount, sMap2));
            Outcome oracleOutcome = Outcome.capture(() -> oracle.examCall(oMap, colors, oStrings, amount, oMap2));
