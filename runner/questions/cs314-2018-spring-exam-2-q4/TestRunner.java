...
public class TestRunner {
    ...
    private static void runCases() {
        for (int caseIndex = 0; caseIndex < 6; caseIndex++) {
            ...
            Map sMap = new LinkedHashMap() {{ put("Ada", new LinkedHashMap<String, Double>() {{ put("CS", 4.0); put("Math", 3.5); }}); put("Bea", new LinkedHashMap<String, Double>() {{ put("CS", 3.0); }}); }};
            ...
            Outcome studentOutcome = Outcome.capture(() -> StudentSolution.highestGPA(sMap));
            Outcome oracleOutcome = Outcome.capture(() -> OracleStudentSolution.highestGPA(oMap));
