...
public class TestRunner {
    ...
    private static void runCases() {
        for (int caseIndex = 0; caseIndex < 6; caseIndex++) {
            ...
            Set<String> sSet = new LinkedHashSet<>(Arrays.asList("a", "at", "ate", "late", "plate", "rate"));
            ...
            String target = (new String[]{"at", "ate", "late", "plate", "rate", "crate"})[caseIndex];
            int amount = caseIndex + 1;
            int start = caseIndex % 2;
            int stop = Math.max(start, Math.min(base.length, start + amount));
            Outcome studentOutcome = Outcome.capture(() -> { StudentSolution.findWords(target, sSet, sStrings); return null; });
            Outcome oracleOutcome = Outcome.capture(() -> { OracleStudentSolution.findWords(target, oSet, oStrings); return null; });
