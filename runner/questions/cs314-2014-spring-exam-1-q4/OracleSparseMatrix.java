...
class OracleSparseMatrix implements ExamSnapshot {
    ...
    public OracleSparseMatrix(int rows, int columns) { set(new int[rows][columns]); }
    ...
    static OracleSparseMatrix of(int[][] source) {
        OracleSparseMatrix result = new OracleSparseMatrix(source.length, source[0].length);
        result.set(source);
        return result;
    }
