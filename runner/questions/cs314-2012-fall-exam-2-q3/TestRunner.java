...
public class TestRunner {
    ...
    private static void runCases() {
        for (int caseIndex = 0; caseIndex < 6; caseIndex++) {
            ...
            String target = (new String[]{"()", "(", "[", "([]", "([)]", "()[[]"})[caseIndex];
            int amount = caseIndex + 1;
            int start = caseIndex % 2;
            int stop = Math.max(start, Math.min(base.length, start + amount));
            Outcome studentOutcome = Outcome.capture(() -> student.getMissingGroupingSymbols(target));
            Outcome oracleOutcome = Outcome.capture(() -> oracle.getMissingGroupingSymbols(target));
