...
public class TestRunner {
    ...
    private static void runCases() {
        for (int caseIndex = 0; caseIndex < 6; caseIndex++) {
            NameRecord314 student = NameRecord314.fixture(caseIndex);
            OracleNameRecord314 oracle = OracleNameRecord314.fixture(caseIndex);
            
            int amount = caseIndex + 1;
            Outcome studentOutcome = Outcome.capture(() -> student.isTrendy(amount));
            Outcome oracleOutcome = Outcome.capture(() -> oracle.isTrendy(amount));
