...
public class TestRunner {
    ...
    private static void runCases() {
        for (int caseIndex = 0; caseIndex < 6; caseIndex++) {
            ...
            Map sMap = new LinkedHashMap() {{ put("A", new ArrayList<>(Arrays.asList("x", "y"))); put("B", new ArrayList<>(Arrays.asList("y"))); }};
            ...
            Outcome studentOutcome = Outcome.capture(() -> student.getMoviesWithMaxReferences(sMap));
            Outcome oracleOutcome = Outcome.capture(() -> oracle.getMoviesWithMaxReferences(oMap));
