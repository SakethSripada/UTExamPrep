...
public class TestRunner {
    ...
    private static void runCases() {
        for (int caseIndex = 0; caseIndex < 6; caseIndex++) {
            ...
            Map sMap = new LinkedHashMap() {{ put("A", new LinkedHashSet<>(Arrays.asList("x", "y"))); put("B", new LinkedHashSet<>(Arrays.asList("y", "z"))); put("C", new LinkedHashSet<>(Arrays.asList("x"))); }};
            ...
            Outcome studentOutcome = Outcome.capture(() -> StudentSolution.highestSimilarity(sMap));
            Outcome oracleOutcome = Outcome.capture(() -> OracleStudentSolution.highestSimilarity(oMap));
