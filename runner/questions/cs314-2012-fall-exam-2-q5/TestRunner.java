...
public class TestRunner {
    ...
    private static void runCases() {
        for (int caseIndex = 0; caseIndex < 6; caseIndex++) {
            ...
            String target = (new String[]{"CAT", "CAR", "TEN", "ARE", "CART", "ZZ"})[caseIndex];
            int amount = caseIndex + 1;
            int start = caseIndex % 2;
            int stop = Math.max(start, Math.min(base.length, start + amount));
            Outcome studentOutcome = Outcome.capture(() -> student.isPresent(target, currentCase % 3, currentCase % 3));
            Outcome oracleOutcome = Outcome.capture(() -> oracle.isPresent(target, currentCase % 3, currentCase % 3));
